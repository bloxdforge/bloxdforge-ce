...
interface DeletePackConfirmationModalProps {
  packName: string;
  onConfirm: () => void;
  onCancel: () => void;
}
export default function DeletePackConfirmationModal({ packName, onConfirm, onCancel }: DeletePackConfirmationModalProps) {
  const { setConfirmPackDeletion } = useSettingsStore();
  const [dontAskAgain, setDontAskAgain] = useState(false);
  const handleConfirm = () => {
    if (dontAskAgain) {
      setConfirmPackDeletion(false);
    }
    onConfirm();
  };
  return (
      ...
      <div className="bg-background border border-surface-border rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6">
          <p className="text-gray-300">
            Are you sure you want to permanently delete the pack <strong className="text-primary">{packName}</strong>? All files inside will be lost. This action cannot be undone.
          </p>
          <div className="flex items-center space-x-3">
            <input
              id="dont-ask-again-pack"
              type="checkbox"
              checked={dontAskAgain}
              onChange={(e) => setDontAskAgain(e.target.checked)}
              className="h-4 w-4 rounded border-gray-500 bg-surface-hover text-primary focus:ring-primary"
            />
            <label htmlFor="dont-ask-again-pack" className="text-sm text-text-muted">
              Don&apos;t ask me again
            </label>
          </div>
